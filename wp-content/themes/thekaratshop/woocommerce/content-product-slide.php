...
<div class="slide">
    <div class="product-item">
        <a href="<?=esc_url( apply_filters( 'woocommerce_loop_product_link', get_the_permalink(), $product ) )?>" aria-label="<?= esc_attr( $product->get_name() ) ?>"></a>
        ...
        if(get_class($product) == 'WC_Product_Variable'){
            ...
            foreach($available_variations as $key => $value){
                if(!empty($value['image']) && !empty($value['attributes']['attribute_pa_color']) && !isset($colors[$value['attributes']['attribute_pa_color']])){
                    ...
                    $images[$value['attributes']['attribute_pa_color']] = '<img fetchpriority="high" '.$itemprop.' loading="lazy" data-color="'.$value['attributes']['attribute_pa_color'].'" decoding="async" width="'.$value['image']['src_w'].'" height="'.$value['image']['src_h'].'" src="'.$value['image']['src'].'" class="attachment- size-" alt="Product '.esc_html($product->get_name()).'" title="Product '.esc_html($product->get_name()).'" srcset="'.$value['image']['srcset'].'" sizes="'.$value['image']['sizes'].'">'.$meta;
                }
            }
        }
        ?>
...
//            do_action( 'woocommerce_before_shop_loop_item_title' );
            ...
            if(!empty($images)){
                ...
                echo $product ? $product->get_image($image_size, ['alt' => $product->get_name(), 'title' => $product->get_name(), 'loading' => 'lazy', 'itemprop' => 'contentUrl'], true) : '';
