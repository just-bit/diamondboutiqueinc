...
class Wishlist_Account_View {
	...
	public static function wishlist_shortcode() {
		...
		$user = get_current_user_id();
		if(!empty($_GET['key'])){
            if(strlen($_GET['key']) == strlen((int)$_GET['key'])){
                $products = $wpdb->get_results("SELECT * FROM `$table_name` where `user_id` = '".$_GET['key']."'", ARRAY_A);
            }else{
                $table_name = $wpdb->prefix . 'wt_guest_wishlists';
                $session_id = WC()->session->get('sessionid');
                $products = $wpdb->get_results("SELECT * FROM `$table_name` where `session_id` = '".$_GET['key']."'", ARRAY_A);
            }
        }else{
            if(is_user_logged_in()){
                $products = $wpdb->get_results("SELECT * FROM `$table_name` where `user_id` = '$user'", ARRAY_A);
