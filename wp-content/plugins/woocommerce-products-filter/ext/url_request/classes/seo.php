...
class WOOF_SEO {
    ...
    public function __construct($rules, $url) {
        ...
        add_filter('wp_robots', array($this, 'wp_robots'), ( \PHP_INT_MAX - 9 ));
        add_filter('wpseo_robots_array', array($this, 'wp_robots'));
    ...
    public function is_search_going() {
        ...
        $is_search = false;
        if (woof()->is_isset_in_request_data(woof()->get_swoof_search_slug()) || !empty($_GET['orderby'])) {
            $is_search = true;
        }
    ...
    public function ajax_page_title($get, $link) {
        ...
        if($this->do_index()){
            if(woof()->get_really_current_term()){
                ...
                set_query_var('product_cat', $g->slug);
            }
            if (!isset($rule['title'])) {
                $rule['title'] = '';
            }
            ...
            $meta_title = $this->pre_get_document_title(apply_filters('woof_seo_meta_title', $this->replace_vars($rule['title'], $this->get_current_replace_vars())));
            $title = $this->set_h1(apply_filters('woof_seo_h1', $this->replace_vars($rule['h1'], $this->get_current_replace_vars())));
    ...
    public function meta_head() {
        ...
        if (!$rule) {
            add_filter('wpseo_metadesc', function($meta_description, $presentation){
                $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
                $request_data = woof()->get_request_data();
                if(isset($request_data['options'])){
                    unset($request_data['options']);
                }
                if(count($request_data) == 1){
                    $taxonomy = key($request_data);
                    $label_name = wc_attribute_label($taxonomy);
                    $term = get_term_by('slug', $request_data[$taxonomy], $taxonomy);
                    if(!empty($term)){
                        if(!empty($cat = get_query_var('product_cat'))) {
                            $category = get_term_by('slug', $cat, 'product_cat');
                            $meta_description = esc_attr($category->name.' '.$label_name.' '.$term->name.'  at the best price ✔️ Only we have the best prices ✔️ We guarantee the quality of our products, see for yourself!');
                            if ($paged > 1) {
                                $meta_description .= ' - Page ' . $paged;
                            }
                            return $meta_description;
                        }else{
                            $meta_description = esc_attr($label_name.' '.$term->name.'  at the best price ✔️ Only we have the best prices ✔️ We guarantee the quality of our products, see for yourself!');
                            if ($paged > 1) {
                                $meta_description .= ' - Page ' . $paged;
                            }
                            return $meta_description;
                    ...
                    if(strpos($_SERVER['REQUEST_URI'], '/shop/') === 0){
                        $meta_description = 'All jewelry at the best price ✔️ Only we have the best prices ✔️ We guarantee the quality of our products, see for yourself!';
                    }elseif(!empty($cat = get_query_var('product_cat'))){
                        $category = get_term_by( 'slug', $cat, 'product_cat');
                        if(!empty($category)) {
                            if (empty($category->parent)) {
                                $meta_description = $category->name . ' at the best price ✔️ Only we have the best prices ✔️ We guarantee the quality of our products, see for yourself!';
                            } else {
                                $meta_description = $category->name . ' at the best price ✔️ Only we have the best prices ✔️ We guarantee the quality of our products, see for yourself!';
                            }
                        }
        ...
        if ($rule['description']) {
            add_filter('wpseo_metadesc', function($meta_description, $presentation) use($rule){
                return esc_attr($rule['description']);
            }, 10, 2);
            //echo sprintf('<meta name="description" content="%s" />', esc_attr($rule['description'])) . "\r\n";
        }else{
            add_filter('wpseo_metadesc', function($meta_description, $presentation){
                return esc_attr('Best price for '.$meta_description);
            }, 10, 2);
        }
    ...
    public function pre_get_document_title($title) {
        ...
        if($rule){
            ...
            if(!defined('DOING_AJAX') && strpos($_SERVER['REQUEST_URI'], '/product-category/') == 0){
                $category_slug = explode('/', str_replace('/product-category/', '', $_SERVER['REQUEST_URI']))[0];
                $category = get_term_by('slug', $category_slug, 'product_cat');
            }else{
                $category = woof()->get_really_current_term();
            }
            if(count($request_data) == 1){
                $taxonomy = key($request_data);
                $label_name = wc_attribute_label($taxonomy);
                ...
                if(!empty($term)){
                    ...
                    if(!empty($category)) {
                        $title = $category->name.' '.$term->name.' '.$label_name . ' buy in online jewelry store - The Karat Shop';
                    }else{
                        $title = $term->name.' '.$label_name . ' buy in online jewelry store - The Karat Shop';
                    }
                }
            }else{
                if(!empty($category)){
                    if(!empty($category)) {
                        if (empty($category->parent)) {
                            $title = $category->name . ' buy in online jewelry store - The Karat Shop';
                        } else {
                            $title = $category->name . ' buy in online jewelry store - The Karat Shop';
                        }
                    }
                }elseif(strpos($_SERVER['REQUEST_URI'], '/shop/') === 0){
                    $title = 'All Jewelry';
                }
            }
        }
    ...
    public function set_h1($title) {
        ...
        if(empty($rule['h1'])){
            ...
            if(count($request_data) == 1){
                ...
                $term = get_term_by('slug', $request_data[$taxonomy], $taxonomy);
                if(!empty($term)){
                    if(!empty($category = woof()->get_really_current_term())) {
//                        $category = get_term_by('slug', $cat, 'product_cat');
                        $title = $category->name.' '.$term->name.($label_name != $term->name ? ' '.$label_name : '');
                    }else{
                        $title = $term->name.($label_name != $term->name ? ' '.$label_name : '');
                    }
                }
            }else{
                if(!empty($category = woof()->get_really_current_term())){
...
//        echo sprintf('<link rel="canonical" href="%s" />', esc_attr($canonical_link)) . "\r\n";
    ...
    public function add_seo_text() {
        ...
        if (!empty($txt) && !get_query_var('paged')) {
            //echo '<div class="woof_seo_text">' . wp_kses_post(wp_unslash($txt)) . "</div>\r\n";
            echo '<div class="catalog-txt__screen">
                <div class="container">
                    <div class="txt-wrapper">' . wp_kses_post(wp_unslash($txt)) . "</div>
                </div>
            </div>\r\n";
        }
    }
