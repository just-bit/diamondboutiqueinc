...
final class WOOF {
    ...
    public function __construct() {
        ...
        if (!defined('DOING_AJAX')) {
            ...
            if(isset($_POST['link'])){
                global $wpdb;
                $parts = explode('/', $_POST['link']);
                foreach($parts as $i => $part){
                    if($part == 'product-category' && isset($parts[$i + 1])){
                        $category = $wpdb->get_row( "SELECT * FROM `wp_terms`, `wp_term_taxonomy` WHERE `slug` LIKE '".$parts[$i + 1]."' AND `wp_terms`.`term_id` = `wp_term_taxonomy`.`term_id` AND `wp_term_taxonomy`.`taxonomy` = 'product_cat'" );
                        if(!empty($category)){
                             $this->storage->set_val($this->session_rct_key, $category);
                        }
                        break;
                    }
                }
...
//
    ...
    public function wp_head() {
        if (!defined('DOING_AJAX') && !is_page()) {
            ...
            if (isset($wp_query->query_vars['taxonomy']) AND is_object($queried_obj) AND get_class(get_queried_object()) == 'WP_Term' AND !isset($request_data['really_curr_tax'])) {
                ...
                if(strpos($_SERVER['REQUEST_URI'], '/product-category/') == 0){
                    $category_slug = explode('/', str_replace('/product-category/', '', $_SERVER['REQUEST_URI']))[0];
                    $category = get_term_by( 'name', $category_slug, 'product_cat' );
                    $this->set_really_current_term(!empty($category) ? $category : null);
                }else{
                     $this->set_really_current_term();
                }
            }
    ...
    public function woocommerce_before_shop_loop() {
        ...
        if (get_option('woof_try_ajax', 0) && !WOOF_REQUEST::isset('woof_products_doing') && !$is_wc_shortcode) {
            echo '<div class="woocommerce woocommerce-page woof_shortcode_output" style="display: flex;flex-direction: column;">';
    ...
    public function woof_products($atts, $is_prediction = false) {
        ...
        <?php if ($is_ajax == 1): ?>
            <div id="woof_results_by_ajax" data-count="<?php echo intval($products->found_posts) ?>" class="woof_results_by_ajax_shortcode 1" data-shortcode="<?php echo esc_attr($shortcode_txt) ?>">
                <?php
                //endif;
                //ajax compatibility
        ...
        public function woof_shortcode($atts) {
            ...
            if(file_exists(get_template_directory() . DIRECTORY_SEPARATOR . 'woof' . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'woof.php')) {
                $html = $this->render_html(get_template_directory() . DIRECTORY_SEPARATOR . 'woof' . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'woof.php', apply_filters('woof_filter_shortcode_args', $args));
            }else{
