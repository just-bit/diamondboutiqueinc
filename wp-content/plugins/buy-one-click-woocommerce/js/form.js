...
function buyone_click_body_scroll() {
    ...
    var formVisible = jQuery('#formOrderOneClick .cart-popup').css('visibility')
...
jQuery(document).ready(function () {
    ...
    jQuery(document).on('submit', '#buyoneclick_form_order', function (e) {
        e.preventDefault();
        if(jQuery(this).find('[name="txtphone"]').val() === ''){
            jQuery(this).find('.input-wrapper').addClass('error').append('<label id="phone-error" class="error" for="phone">This field is required</label>');
            return false;
        }else{
            jQuery(this).find('.input-wrapper').removeClass('error').find('#phone-error').remove();
        }
        ...
        jQuery.ajax({
            ...
            if (buyone_ajax.success_action === 1) { //Действие по умолчанию
                ...
                gtag("event", "purchase", {
                    transaction_id: orderIdenty,
                    value: total,
                    items: items,
                    currency: "USD",
                    send_to: "G-752ZFM0122"
                })
                // jQuery('#formOrderOneClick .close_order').trigger('click');
                ...
                // jQuery('#formOrderOneClick .mfp-container').remove();
    ...
    jQuery(document).on('click', 'button.clickBuyButton', function (e) {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                jQuery('.order-one-click__product').html(response.data);

                window.open_one_click_form();

                // jQuery('.cart-popup').css('opacity', '1');
                // jQuery('.cart-popup').css('visibility', 'visible');
