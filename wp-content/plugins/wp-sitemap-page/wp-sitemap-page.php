...
function wsp_return_content_type_page($is_title_displayed = true, $is_get_only_private = false, $display_nofollow = false, $wsp_exclude_pages = array(), $sort = null) {
	...
	// get data
	$total = ceil(count(get_pages($args)) / $per_page);
    $args['number'] = $per_page;
    $args['offset'] = ((get_query_var('paged')) ? get_query_var('paged') - 1 : 0) * $args['number'];
    ...
    if($pages){
        $return .= '<div class="pagination" style="margin-top: 40px;"><nav><ul>';
        foreach ($pages as $link) {
            if (strpos($link, 'current') !== false) {
                $return .= '<li class="current">' . str_replace('/page/1/', '/', $link) . '</li>';
            } else {
                $return .= '<li>' . str_replace('/page/1/', '/', $link) . '</li>';
            }
        }
        $return .= '</ul></nav></div>';
    }
...
function wsp_return_content_type_taxonomy_items( $is_title_displayed=true, $display_nofollow=false, $taxonomy_obj=null, $wsp_exclude_pages='' ) {
	...
	$total = ceil(count(get_terms($taxonomies, $args)) / $per_page);
    $args['number'] = $per_page;
    $args['offset'] = $current * $args['number'] - $args['number'];
